(() => {
  ...
  const fallbackI18n = {
    'zh-CN': {
      ...
      'progress.toggleStatus': '点击切换已翻译/未翻译状态',
      'context.viewMode': '显示模式',
      'context.singleMode': '单条模式',
      'context.groupMode': '上下文组模式',
      'context.noGroups': '当前文件暂无上下文组',
      'context.groupTargetPlaceholder': '输入该上下文组的整体译文',
      'context.groupLineCount': '计划断成几行',
      'context.groupBoundaryTitle': '建议切分点',
      'context.groupPreviewLabel': '译文预览（点击行间分隔线插入/移除分行标记）',
      'context.groupSplitHint': '在上方组合译文中用 ---SPLIT--- 标记分行位置，应用时会按标记或换行回填到所选条目。',
      'context.groupSplitOnApply': '回填时按换行拆分',
      'context.groupApply': '应用到所选',
      'context.groupClear': '清空选择',
      'context.groupShort': '上下文组',
      'glossary.hitCount': '术语命中 {count} 条',
      'glossary.untitledSource': '未命名原文',
      'glossary.untitledTarget': '未命名译文',
      'glossary.insertMode': '插入方式',
      ...
      'build.testVersion': '测试版',
      'build.stableVersion': '稳定版'
    },
    en: {
  ...
  function bindShellActions() {
    ...
    $('saveTraditionalSettingsBtn')?.addEventListener('click', () => saveTraditionalSettings().catch((e) => setStatusBox('traditionalStatus', e.message || '保存失败', 'error', '传统翻译设置')));
    $('testTraditionalBtn')?.addEventListener('click', () => testTraditionalSettings().catch((e) => setStatusBox('traditionalStatus', e.message || '测试失败', 'error', '传统翻译测试')));
    $('saveAiSettingsBtn')?.addEventListener('click', () => saveAiSettings().catch((e) => setStatusBox('aiStatus', e.message || '保存失败', 'error', '大模型翻译设置')));
    $('testAiBtn')?.addEventListener('click', () => testAiSettings().catch((e) => setStatusBox('aiStatus', e.message || '测试失败', 'error', '大模型翻译测试')));
